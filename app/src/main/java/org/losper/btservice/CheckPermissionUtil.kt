...
object CheckPermissionUtil {
    private val mNeedPermissions = arrayOf(Manifest.permission.BLUETOOTH,Manifest.permission.BLUETOOTH_ADMIN)//填入需要检测的权限
    private const val PERMISSION_REQUEST_CODE = 0//权限获取值
    /**
     * 开始检查权限
     */
    fun check(activity: Activity): Boolean {
        val needRequestPermissionList = findDeniedPermissions(activity, mNeedPermissions)
        Log.d("BtService:Test", ""+needRequestPermissionList.size)
    ...
    private fun findDeniedPermissions(activity: Activity, permissions: Array<String>): List<String> {
        return permissions.filter {
            ContextCompat.checkSelfPermission(activity, it) != PackageManager.PERMISSION_GRANTED || ActivityCompat.shouldShowRequestPermissionRationale(activity, it) }
    }
